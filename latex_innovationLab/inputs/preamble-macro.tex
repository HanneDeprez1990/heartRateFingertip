...
\newcommand{\titel}{Je hartslag monitor altijd op zak}
...
{\theoremstyle{mijnopmerking}
	\newtheorem*{onthoud}{Onthoud}}

\newenvironment{ftonthoud}
{\begin{mdframed}\begin{onthoud}}
		{\end{onthoud}\end{mdframed}}
...
{\theoremstyle{mijnvoorbeeld}
	\newtheorem{voorbeeld}{Voorbeeld}[subsection]}
...
%%%%%%%%% Omgevingen waarin de eningen (oef en steroef) worden getypeset. %%%%%%%
...
\begin{tikzpicture}
\fill[fill=fac-color-orange] (0,\streepdikte/2) rectangle (\linkerlijn,-\streepdikte/2);
\node at (\linkerlijn+2ex,0) [anchor=west,inner sep=0pt] {{\bf\large\boldmath#1}};
\fill[fill=fac-color-orange] (\textwidth-\overschot,\streepdikte/2) rectangle (\textwidth,-\streepdikte/2);
\end{tikzpicture}}
